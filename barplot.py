...
def barplot_creator(analysis):
    ...
    plt.title(f"Fold changes of top {analysis.top_n} upregulated"
              f"{analysis.expression_values1} vs {analysis.expression_values2}", size=label_size)
    # save the plot as pdf with the f string formatted passed through arguments as the names
    plot1.savefig(
        f"top-{analysis.top_n}-upregulated"
        f"-{analysis.expression_values1}-vs-{analysis.expression_values2}.pdf")
    # creating a plot with a certain figuresize by using the imported module
    ...
    # add a title to the plot with the f string formatted
    # passed through arguments from the command line
    plt.title(f"Fold changes top {analysis.top_n} downregulated {analysis.expression_values1}"
              f"vs {analysis.expression_values2}", size=label_size)
    # save the plot as pdf with the f string formatted passed through arguments as the names
    plot2.savefig(
        f"top-{analysis.top_n}-downregulated"
        f"-{analysis.expression_values1}-vs-{analysis.expression_values2}.pdf")
